...
/**
 * Handles actions related to participants, such as profile management and exam interaction.
 */
class ParticipantController extends Controller
{
    /**
     * Show the participant's profile form.
     *
    ...
    /**
     * Store the participant's profile data.
     *
    ...
    /**
     * Launch the exam room for the participant.
     *
     * This method handles the logic for starting or resuming an exam, loading the
     * necessary data for the participant's current state.
     *
    ...
    /**
     * Display a page indicating that the participant has no active exam.
     *
     * @return \Inertia\Response
     */
    public function noExam(): Response
    {
        return Inertia::render('Exams/NoExam');
    }

    /**
     * Mark an exam step as started for the participant.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function startStep(Request $request): RedirectResponse
    {
        $user = Auth::user();
    ...
    /**
     * Mark an exam step as completed and save the results.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    ...
    /**
     * Mark an exam step as broken/aborted by the participant.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function breakStep(Request $request): RedirectResponse
    {
        $user = Auth::user();
        $examStepStatus = ExamStepStatus::where('participant_id', $user->id)
            ->where('exam_step_id', $request->input('exam_step_id'))
    ...
    /**
     * List all participants for the current user's city.
     *
     * @return \Inertia\Response
     */
    ...
    /**
     * Save the participant's progress when a step is paused.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function pauseStep(Request $request): RedirectResponse
    {
        $user = Auth::user();
        $examStepStatus = ExamStepStatus::with('step.test')
            ->where('participant_id', $user->id)
