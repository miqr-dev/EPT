...
<template>
  <div>
    <h2 class="text-2xl font-bold mb-4">Prüfung erstellen</h2>
    <form @submit.prevent="saveExam" class="space-y-6">
      <div>
        <label for="name" class="block text-sm font-medium text-gray-700">Prüfungstitel</label>
      ...
      <div>
        <label for="teacher" class="block text-sm font-medium text-gray-700">Prüfer:in</label>
      ...
      <div>
        <h3 class="text-lg font-medium text-gray-700">Prüfungsschritte</h3>
        <div v-for="(step, index) in form.steps" :key="index" class="flex items-center space-x-4 mt-2">
          ...
          <button type="button" @click="removeStep(index)" class="text-red-600 hover:underline">Entfernen</button>
        </div>
        <button type="button" @click="addStep" class="mt-2 text-blue-600 hover:underline">Schritt hinzufügen</button>
      </div>
