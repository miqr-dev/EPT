...
const rowBgClass = (qnum: number) => (isAnswered(qnum) ? '' : 'bg-slate-50 dark:bg-slate-800/40');
const borderClass = (qnum: number) => (isAnswered(qnum) ? 'border-slate-100 dark:border-slate-800' : 'border-slate-300 dark:border-slate-500');
</script>
        ...
        <div class="flex min-h-[600px] flex-1 gap-4 rounded-xl bg-muted/20 p-4">
            <div class="flex flex-1 flex-col gap-4">
                ...
                <div v-if="!showTest" class="flex h-full flex-col items-center justify-center">
                    ...
                    <p class="mb-12 w-full max-w-4xl text-center text-xl">
                        Wir bitten Sie, einige Ihrer üblichen Verhaltensweisen, Einstellungen und Gewohnheiten zu beschreiben, wobei vor allem auf Ihr
                        Arbeitsleben Bezug genommen wird. Dazu finden Sie im Folgenden eine Reihe von Aussagen. Lesen Sie jeden dieser Sätze gründlich
                        durch und entscheiden Sie, in welchem Maße er auf Sie persönlich zutrifft.
                    </p>
                    ...
                    <div class="w-full max-w-4xl">
                        ...
                        <div class="grid grid-cols-1 gap-6 text-xl sm:grid-cols-2">
                            <!-- Left note box -->
                            ...
                            <div class="flex flex-col">
                                ...
                                <div class="grid grid-cols-5 items-end gap-4">
                                    <div v-for="item in LEGEND_TOP" :key="item.val" class="flex flex-col items-center">
                                        ...
                                        <div class="mb-1 text-center text-[13px] text-gray-800 dark:text-gray-200">
                                            {{ item.text }}
                                        </div>
                                ...
                                <div class="mt-3 text-sm text-gray-700 italic dark:text-gray-300">
                ...
                <!-- Test table -->
                <div v-else class="w-full self-center overflow-x-auto rounded-lg border bg-background p-6 md:w-3/4">
                    <table class="w-full border-separate text-base" style="border-spacing: 0">
                        <tbody>
                            <tr v-for="q in AVEM_QUESTIONS" :key="q.number" :class="rowBgClass(q.number)">
                                        ...
                                        <TooltipProvider v-if="PARTNER_INFO_QUESTION_NUMBERS.has(q.number)" :delay-duration="0">
                                            <Tooltip>
                                                <TooltipTrigger as-child>
                                                    <button
                                                        type="button"
                                                        class="mt-0.5 text-blue-500 transition hover:text-gray-800 focus:ring-2 focus:ring-primary focus:outline-none dark:text-gray-400 dark:hover:text-gray-200"
                                ...
                                >
                                    <label
                                        class="relative inline-flex h-7 w-7 cursor-pointer items-center justify-center rounded-full ring-offset-2 ring-offset-background transition hover:scale-[1.04]"
                                        :class="
                                    ...
                                    >
                                        ...
                                        <svg
                                            class="h-7 w-7"
                                            :class="
                                                answers[q.number] === opt ? 'text-blue-600 dark:text-blue-400' : 'text-gray-700 dark:text-gray-200'
                                            "
                                            viewBox="0 0 24 24"
                                            aria-hidden="true"
                                        >
