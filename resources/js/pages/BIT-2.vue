      ...
      <div class="flex flex-1 flex-col gap-4">
        <div v-if="!showTest" class="flex h-full flex-col items-center justify-center">
          ...
          <p class="mb-6 text-xl text-center max-w-xl">
            Dieser Fragebogen soll ein Bild Ihrer Interessen an verschiedenen beruflichen Arbeiten vermitteln.
            Dafür werden Ihnen 81 Tätigkeiten aufgeführt. Beurteilen Sie diese der Reihe nach. Sie haben dafür
            die Ziffer
          </p>

          <!-- This block is now aligned like the text above, but left inside -->
          <div class="w-full max-w-xl text-left">
            <p class="text-xl">5 sehr gern</p>
            <p class="text-xl">4 gern</p>
            <p class="text-xl">3 weder gern noch ungern</p>
            <p class="text-xl">2 ungern</p>
            <p class="text-xl">1 sehr ungern</p>

            <p class="mt-4 text-xl">zur Verfügung.</p>
          </div>

          <Button @click="startTest" class="mt-8 rounded-xl px-8 py-3 text-lg font-semibold shadow">
            Test starten
          </Button>
        </div>
            ...
            <table class="w-full border-separate text-base" style="border-spacing: 0">
              <tbody>
                <tr v-for="q in firstPageQuestions" :key="q.number"
                  :class="{ 'bg-gray-50 dark:bg-gray-700': answers[q.number] === null }">
                  <td
                    class="font-mono w-8 pr-2 align-top pt-2 text-right border-b-2 border-gray-200 dark:border-gray-700">
                    {{ q.number }}.</td>
                  <td class="pr-4 align-top pt-2 border-b-2 border-gray-200 dark:border-gray-700">{{ q.text }}</td>
                  <td v-for="opt in [5, 4, 3, 2, 1]" :key="opt"
                    class="px-2 text-center align-top pt-1 border-b-2 border-gray-200 dark:border-gray-700">
                    <label
                      class="w-8 h-8 relative flex items-center justify-center border border-gray-400 cursor-pointer">
                      <input class="sr-only" type="radio" :name="'q' + q.number" :value="opt"
                        v-model="answers[q.number]" />
                      <span class="text-sm">{{ opt }}</span>
                      <span v-if="answers[q.number] === opt"
                        class="absolute inset-0 flex items-center justify-center text-red-600 text-xl">X</span>
                    </label>
                  </td>
                </tr>
              </tbody>
