...
const questions = ref<Question[]>([
    { text: '619020 – 541600 = ?', answers: ['77420','77,420','77.420',] },
    { text: '619020 = 174309 + ?', answers: ['444711','444.711','444,711'] },
    ...
    {
        text: 'Ein Mechaniker hat aus 3 Teilen ein Gerät hergestellt. Die Einzelteile wiegen: 50 g, 9,4 kg, 1050 g. Wie viel wiegt das gesamte Gerät?',
        answers: ['10500', '10500 g', '10500g', '10,5', '10,5Kg', '10,5kg', '10,5 Kg', '10,5 kg'],
    },
    ...
    {
        text: 'Rudi kauft sich ein neues Mofa. Es kostet 1390 €. Bei Barzahlung bekommt er 2 % Rabatt. Wie viel muss er bezahlen?',
        answers: ['1362,2', '1362,20', '1362,20 €', '1362,2 €', '1362,2€', '1362,20€'],
    },
    {
        text: 'Im Sägewerk können aus einem Baumstamm 20 Bretter von 3 cm Dicke geschnitten werden. Wie viele Bretter erhält man, wenn sie 2 cm dick sind?',
        answers: ['30'],
    },
    {
        text: 'Im Sägewerk können aus einem Baumstamm 20 Bretter von 3 cm Dicke geschnitten werden. Wie dick wird ein Brett, wenn man 15 Bretter aus dem Stamm schneidet?',
        answers: ['4', '4cm', '4 cm', '4 Cm', '4Cm'],
    },
...
function formatQuestionMark(text: string): string {
    if (text.endsWith('?')) {
        return text.slice(0, -1) + '<span class="font-bold text-red-600 text-lg">?</span>';
    }
    return text;
}
        ...
        <div class="flex min-h-[600px] flex-1 gap-4 rounded-xl bg-muted/20 p-4">
            ...
            <aside v-if="showTest" class="sticky top-8 flex h-fit w-64 flex-shrink-0 flex-col items-start space-y-2 py-4">
                ...
                <div class="flex w-full flex-col items-start space-y-1">
                    <template v-for="(q, idx) in questions" :key="idx">
                        <button
                            class="flex w-full items-center space-x-2 rounded-lg border px-2 py-1 text-base font-medium transition hover:bg-blue-50 focus:outline-none"
                            :class="{
                                'border-blue-600 bg-blue-600 text-white': idx === currentQuestionIndex,
                                'hover:bg-blue-500': idx === currentQuestionIndex,
                                'border-gray-400 bg-gray-300 text-gray-900': userAnswers[idx] && idx !== currentQuestionIndex,
                                'border-gray-300 bg-gray-100 text-gray-900': !userAnswers[idx] && idx !== currentQuestionIndex,
                            }"
                        ...
                        >
                            <span
                                ...
                                :class="{
                                    'border-blue-600 bg-blue-600 text-white': idx === currentQuestionIndex,
                                    'border-gray-400 bg-gray-400 text-white': userAnswers[idx] && idx !== currentQuestionIndex,
                                    'border-gray-400 bg-gray-300 text-gray-600': !userAnswers[idx] && idx !== currentQuestionIndex,
                                }"
                            >
                                {{ idx + 1 }}
                            </span>
            ...
            <div class="flex flex-1 flex-col gap-4">
                ...
                <div v-if="!showTest" class="flex h-full flex-col items-center justify-center">
                    ...
                    <p class="mb-6 max-w-xl text-center text-xl">
                        In diesem Verfahren finden Sie insgesamt {{ questions.length }} Rechenaufgaben, die zu lösen sind. Hierfür haben Sie 35
                        Minuten Zeit. Halten Sie sich nicht zu lange an einer Aufgabe auf, wenn Sie sie nicht lösen können. Gehen Sie zur nächsten
                        weiter.
                    </p>
                    <p class="text-xl">
                    ...
                    <br />
                    <p class="text-xl">Für Nebenrechnungen haben Sie einen zusätzlichen Block.</p>
                    <p class="text-xl">Bitte notieren Sie vor Abgabe Ihres Blattes Ihren Namen und das heutige Datum darauf.</p>

                    <Button @click="startTest" class="mt-6 rounded-xl px-8 py-3 text-lg font-semibold shadow"> Test starten </Button>
                </div>
