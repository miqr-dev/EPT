...
const props = defineProps<{
    ...
    pausedTestResult?: Record<string, any> | null;
}>();
...
type BtAnswerTimes = {
    ...
    cash_answers: Record<string, number>;
    folder_answers: Record<string, number>;
    stamp_answer_days: Record<string, number>;
    route_assignments: Record<string, number>;
    route_times: Record<string, number>;
    route_totals: Record<string, number>;
};

type BtAnswerSnapshot = {
    assignments: Record<string, string | null>;
    cash_answers: Record<string, string>;
    folder_answers: Record<string, string>;
    stamp_answer_days: Record<string, boolean>;
    route_assignments: Record<string, string | null>;
    route_times: Record<string, string>;
    route_totals: Record<string, string>;
};

const days = ['Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag'];
const earlySlots = [1, 2];
const lateSlots = [1, 2, 3];
const firstWindowSeconds = 30 * 60;
...
const cashAnswers = ref<Record<string, string>>(Object.fromEntries(cashDenominations.map((denomination) => [denomination.key, ''])));
const folderAnswers = ref<Record<number, string>>(Object.fromEntries(Array.from({ length: 10 }, (_, index) => [index + 1, ''])));
const stampUsage = [
...
const stampAnswerDays = ref<Record<number, boolean>>(Object.fromEntries(Array.from({ length: 10 }, (_, index) => [index + 1, false])));
...
const answerTimes = ref<BtAnswerTimes>(buildEmptyAnswerTimes());
const firstWindowSnapshot = ref<BtAnswerSnapshot | null>(null);
...
function buildBaseSnapshot(): BtAnswerSnapshot {
    return {
        ...
        cash_answers: Object.fromEntries(cashDenominations.map((denomination) => [denomination.key, ''])),
        folder_answers: Object.fromEntries(Array.from({ length: 10 }, (_, index) => [String(index + 1), ''])),
        stamp_answer_days: Object.fromEntries(Array.from({ length: 10 }, (_, index) => [String(index + 1), false])),
        route_assignments: Object.fromEntries(
            routeRows.flatMap((row) => [
                [`route-from-${row}`, row === 1 ? 'Eigene Wohnung' : null],
                [`route-to-${row}`, null],
            ]),
        ),
        route_times: Object.fromEntries(
            routeRows.flatMap((row) => [
                [`route-time-${row}`, ''],
                [`route-msg-${row}`, ''],
            ]),
        ),
        route_totals: { totalWay: '', totalMsg: '', returnWay: '' },
    };
}
...
function normalizeNullableStringRecord(value: unknown) {
    if (!value || typeof value !== 'object') return {};
    return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, entry == null || entry === '' ? null : String(entry)]));
}

function normalizeBooleanRecord(value: unknown) {
    if (!value || typeof value !== 'object') return {};
    return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, entry === true || entry === 'true']));
}

function normalizeTimeRecord(value: unknown) {
    if (!value || typeof value !== 'object') return {};
...
function normalizeAnswerTimes(value: unknown): BtAnswerTimes {
    const source = value && typeof value === 'object' ? (value as Partial<BtAnswerTimes>) : {};
    return {
        assignments: normalizeTimeRecord(source.assignments),
        cash_answers: normalizeTimeRecord(source.cash_answers),
        folder_answers: normalizeTimeRecord(source.folder_answers),
        stamp_answer_days: normalizeTimeRecord(source.stamp_answer_days),
        route_assignments: normalizeTimeRecord(source.route_assignments),
        route_times: normalizeTimeRecord(source.route_times),
        route_totals: normalizeTimeRecord(source.route_totals),
    };
}

function normalizeSnapshot(value: unknown, base: BtAnswerSnapshot = buildBaseSnapshot()): BtAnswerSnapshot {
    const source = value && typeof value === 'object' ? (value as Partial<BtAnswerSnapshot>) : {};
    return {
        assignments: { ...base.assignments, ...normalizeNullableStringRecord(source.assignments) },
        cash_answers: { ...base.cash_answers, ...normalizeStringRecord(source.cash_answers) },
        folder_answers: { ...base.folder_answers, ...normalizeStringRecord(source.folder_answers) },
        stamp_answer_days: { ...base.stamp_answer_days, ...normalizeBooleanRecord(source.stamp_answer_days) },
        route_assignments: { ...base.route_assignments, ...normalizeNullableStringRecord(source.route_assignments) },
        route_times: { ...base.route_times, ...normalizeStringRecord(source.route_times) },
        route_totals: { ...base.route_totals, ...normalizeStringRecord(source.route_totals) },
    };
}

function buildCurrentSnapshot(): BtAnswerSnapshot {
    return {
        assignments: { ...assignments.value },
        cash_answers: { ...cashAnswers.value },
        folder_answers: Object.fromEntries(Object.entries(folderAnswers.value).map(([key, value]) => [key, String(value ?? '')])),
        stamp_answer_days: Object.fromEntries(Object.entries(stampAnswerDays.value).map(([key, value]) => [key, value === true])),
...
function applySnapshot(snapshot: BtAnswerSnapshot) {
    assignments.value = { ...snapshot.assignments };
    cashAnswers.value = { ...snapshot.cash_answers };
    folderAnswers.value = { ...snapshot.folder_answers };
    stampAnswerDays.value = { ...snapshot.stamp_answer_days };
    routeAssignments.value = { ...snapshot.route_assignments };
    routeTimes.value = { ...snapshot.route_times };
    routeTotals.value = { ...snapshot.route_totals };
}

function getFirstWindowSnapshotForScoring() {
    return firstWindowSnapshot.value ? normalizeSnapshot(firstWindowSnapshot.value) : buildCurrentSnapshot();
}

function hasReachedFirstWindow() {
    const elapsed = getElapsedSeconds();
    return elapsed != null && elapsed >= firstWindowSeconds;
}

function ensureFirstWindowSnapshot() {
    if (!firstWindowSnapshot.value) {
        firstWindowSnapshot.value = buildCurrentSnapshot();
    }
}

function ensureFirstWindowSnapshotBeforeChange() {
    if (hasReachedFirstWindow()) {
        ensureFirstWindowSnapshot();
    }
}

function markAnswerTime(section: keyof BtAnswerTimes, key: string | number, value: unknown) {
    const normalizedKey = String(key);
    const hasAnswer = value !== null && value !== undefined && value !== '' && value !== false;
    if (!hasAnswer) {
        delete answerTimes.value[section][normalizedKey];
        return;
    }

    answerTimes.value[section][normalizedKey] = getElapsedSeconds() ?? 0;
}

function clearAnswerTime(section: keyof BtAnswerTimes, key: string | number) {
    delete answerTimes.value[section][String(key)];
}

function answerWasAfterFirstWindow(section: keyof BtAnswerTimes, key: string | number) {
    const timestamp = answerTimes.value[section]?.[String(key)];
    return typeof timestamp === 'number' && timestamp >= firstWindowSeconds;
}

function answerTextClass(section: keyof BtAnswerTimes, key: string | number) {
    return answerWasAfterFirstWindow(section, key) ? 'text-blue-700' : 'text-black';
}
...
function handleDropOnCell(event: DragEvent, key: string) {
    ...
    if (payload.from === 'cell' && payload.key) {
        ...
        clearAnswerTime('assignments', payload.key);
    }

    assignments.value[key] = payload.name;
    markAnswerTime('assignments', key, payload.name);
}
...
function clearCell(key: string) {
    ...
    clearAnswerTime('assignments', key);
}
...
function toggleStampAnswer(day: number) {
    ...
    markAnswerTime('stamp_answer_days', day, stampAnswerDays.value[day]);
}
...
function handleRouteTimeInput(key: string, event: Event) {
    ...
    markAnswerTime('route_times', key, cleaned);
    target.value = cleaned;
}
...
function handleRouteTotalInput(key: 'totalWay' | 'totalMsg' | 'returnWay', event: Event) {
    ...
    markAnswerTime('route_totals', key, cleaned);
    target.value = cleaned;
}

function clearRouteAssignment(key: string) {
    ensureFirstWindowSnapshotBeforeChange();
    routeAssignments.value[key] = null;
    clearAnswerTime('route_assignments', key);
}

function restorePausedBtResult() {
    if (!props.pausedTestResult) return;

    const paused = props.pausedTestResult;
    applySnapshot(normalizeSnapshot(paused));
    answerTimes.value = normalizeAnswerTimes(paused.answer_times);
    firstWindowSnapshot.value = paused.first_30_minute_answers ? normalizeSnapshot(paused.first_30_minute_answers) : null;

    const savedPage = Number(paused.page ?? 1);
    page.value = Number.isFinite(savedPage) ? Math.min(maxPage, Math.max(1, Math.floor(savedPage))) : 1;
    showTest.value = true;

    const savedStartRemaining = Number(paused.started_time_remaining_seconds);
    if (Number.isFinite(savedStartRemaining)) {
        startTimeRemainingSeconds.value = Math.max(0, Math.floor(savedStartRemaining));
    }

    const savedElapsed = Number(paused.total_time_seconds);
    if (Number.isFinite(savedElapsed)) {
        startedAtMs.value = Date.now() - Math.max(0, Math.floor(savedElapsed)) * 1000;
    }
}

restorePausedBtResult();
...
watch(
    ...
    (value) => {
        ...
        if (showTest.value && hasReachedFirstWindow()) {
            ensureFirstWindowSnapshot();
        }
    },
    { immediate: true },
...
function evaluateQ4(snapshot: BtAnswerSnapshot = buildCurrentSnapshot()) {
    const answers = Array.from({ length: 10 }, (_, index) => normalizeFolderLetters(snapshot.folder_answers[String(index + 1)] ?? ''));
...
function evaluateBtSnapshot(snapshot: BtAnswerSnapshot) {
    ...
    routeRows.forEach((row) => {
        const fromRaw = (snapshot.route_assignments[`route-from-${row}`] ?? '').trim();
        const toRaw = (snapshot.route_assignments[`route-to-${row}`] ?? '').trim();
    ...
    const totalWay = Number(snapshot.route_totals.totalWay);
    const totalMsg = Number(snapshot.route_totals.totalMsg);
    const returnWay = Number(snapshot.route_totals.returnWay);
    if (snapshot.route_totals.totalWay !== '' || snapshot.route_totals.totalMsg !== '' || snapshot.route_totals.returnWay !== '') {
        hasAnyInput = true;
    }
    ...
    orderedSlots.forEach(({ shift, slot }) => {
        days.forEach((day) => {
            ...
            const name = snapshot.assignments[key];
    ...
    const q3CorrectFields = Object.entries(q3ExpectedCashCounts).reduce((sum, [key, expected]) => {
        const actual = Number(snapshot.cash_answers[key] || 0);
        return sum + (actual === expected ? 1 : 0);
    }, 0);

    const selectedDays = Object.entries(snapshot.stamp_answer_days)
...
const totalScoring = computed(() => evaluateBtSnapshot(buildCurrentSnapshot()));
const firstWindowScoring = computed(() => evaluateBtSnapshot(getFirstWindowSnapshotForScoring()));
const debugScores = totalScoring;

function cloneAnswerTimes() {
    return {
        assignments: { ...answerTimes.value.assignments },
        cash_answers: { ...answerTimes.value.cash_answers },
        folder_answers: { ...answerTimes.value.folder_answers },
        stamp_answer_days: { ...answerTimes.value.stamp_answer_days },
        route_assignments: { ...answerTimes.value.route_assignments },
        route_times: { ...answerTimes.value.route_times },
        route_totals: { ...answerTimes.value.route_totals },
    };
}

const btResultPayload = computed(() => {
    const currentSnapshot = buildCurrentSnapshot();
    const firstWindowAnswers = firstWindowSnapshot.value ? normalizeSnapshot(firstWindowSnapshot.value) : null;

    return {
        page: page.value,
        ...currentSnapshot,
        answer_times: cloneAnswerTimes(),
        first_30_minute_answers: firstWindowAnswers,
        scoring: {
            ...totalScoring.value,
            first_30_minutes: firstWindowScoring.value,
            total: totalScoring.value,
        },
        total_time_seconds: getElapsedSeconds(),
        started_time_remaining_seconds: startTimeRemainingSeconds.value,
    };
});
...
<template>
    ...
    <div class="relative h-screen overflow-hidden bg-background text-black">
        ...
        <div v-else-if="page === 1" class="flex h-full flex-col">
                    ...
                    <div class="flex flex-1 gap-4">
                        <div class="flex-1 border border-black/20 px-6 py-4">
                            <div class="space-y-4 text-center text-xl leading-relaxed">
                                <p>
                                    Unser Betrieb beschäftigt 25 Lehrlinge. Von diesen sollen jeweils zwei Lehrlinge für den Post-Frühdienst und drei
                                    Lehrlinge für den Post-Spätdienst eingeteilt werden, so dass in jeder Woche jeder Lehrling einmal Postdienst hat.
                                    Aus verkehrstechnischen Gründen können acht Lehrlinge keinen Frühdienst und neun Lehrlinge keinen Spätdienst
                                    machen.
                                </p>
                        ...
                        <div class="flex-1 border border-black/20 px-4 py-2">
                            <div class="flex h-full items-start justify-center gap-8 overflow-hidden">
                                <div
                                    class="w-[420px] flex-none border-2 border-black px-4 py-2 text-xl leading-tight"
                                    @dragover="allowDrop"
                                    ...
                                    <div class="grid grid-cols-2 gap-x-8">
                                        <div class="space-y-0">
                                            <div v-for="apprentice in leftNames" :key="apprentice.id" class="flex items-center gap-2">
                                                ...
                                                <span class="w-4 flex-none text-center">{{ apprentice.restriction ?? '' }}</span>
                                            </div>
                                        </div>
                                        <div class="space-y-0">
                                ...
                                <div class="min-w-[200px] space-y-6 text-left text-xl leading-tight">
                                    <div class="text-center">
                                        <p class="mb-1 text-xl" style="letter-spacing: 0.4em">L i s t e</p>
                                        <p>der Namen</p>
                                    </div>
                                    <div class="space-y-0">
            ...
            <div class="flex-[0.4] overflow-hidden px-6 pt-2 pb-4 font-serif">
                                    ...
                                    >
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                ...
                                                :class="['min-w-0 flex-1 cursor-move', answerTextClass('assignments', buildCellKey('early', 1, day))]"
                                                draggable="true"
                                                @dragstart="
                                                    (event) =>
                                                        handleDragStart(event, {
                                                            name: assignments[buildCellKey('early', 1, day)] ?? '',
                                                            from: 'cell',
                                                            key: buildCellKey('early', 1, day),
                                                        })
                                                "
                                            >
                                    ...
                                    >
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                ...
                                                :class="['min-w-0 flex-1 cursor-move', answerTextClass('assignments', buildCellKey('early', 2, day))]"
                                                draggable="true"
                                                @dragstart="
                                                    (event) =>
                                                        handleDragStart(event, {
                                                            name: assignments[buildCellKey('early', 2, day)] ?? '',
                                                            from: 'cell',
                                                            key: buildCellKey('early', 2, day),
                                                        })
                                                "
                                            >
                                    ...
                                    >
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                ...
                                                :class="['min-w-0 flex-1 cursor-move', answerTextClass('assignments', buildCellKey('late', 1, day))]"
                                                draggable="true"
                                                @dragstart="
                                                    (event) =>
                                                        handleDragStart(event, {
                                                            name: assignments[buildCellKey('late', 1, day)] ?? '',
                                                            from: 'cell',
                                                            key: buildCellKey('late', 1, day),
                                                        })
                                                "
                                            >
                                    ...
                                    >
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                ...
                                                :class="['min-w-0 flex-1 cursor-move', answerTextClass('assignments', buildCellKey('late', 2, day))]"
                                                draggable="true"
                                                @dragstart="
                                                    (event) =>
                                                        handleDragStart(event, {
                                                            name: assignments[buildCellKey('late', 2, day)] ?? '',
                                                            from: 'cell',
                                                            key: buildCellKey('late', 2, day),
                                                        })
                                                "
                                            >
                                    ...
                                    >
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                ...
                                                :class="['min-w-0 flex-1 cursor-move', answerTextClass('assignments', buildCellKey('late', 3, day))]"
                                                draggable="true"
                                                @dragstart="
                                                    (event) =>
                                                        handleDragStart(event, {
                                                            name: assignments[buildCellKey('late', 3, day)] ?? '',
                                                            from: 'cell',
                                                            key: buildCellKey('late', 3, day),
                                                        })
                                                "
                                            >
        ...
        <div v-else-if="page === 2" class="flex h-full flex-col px-12 py-8 font-serif text-xl">
            ...
            <div class="mt-10 flex flex-1 flex-col items-center">
                <div class="max-w-2xl space-y-4 text-center leading-relaxed">
                    <p>Sie erhalten ungeordnet 220 Kopien von Rechnungen aus den Orten Essen, Düsseldorf und Köln.</p>
                    ...
                    <div class="flex flex-1 gap-4">
                        <div class="flex-1 border border-black/20 px-6 py-4">
                            <div class="space-y-4 text-center text-base leading-relaxed">
                                <p class="text-lg">Sie sollen jeden der acht Beträge auf nebenstehender Liste gegen Quittung in bar auszahlen.</p>
                                <p class="text-lg">
                                    Rechnen Sie bitte aus, welche Geldsorten Sie benötigen. Sie sollen in keinem Falle wechseln und möglichst wenig
                                    Geldstücke bzw. Scheine brauchen.
                                </p>
                                <p class="text-lg">Tragen Sie unten ein, welche Anzahl Sie hierzu von jeder Geldsorte benötigen!</p>
                            </div>
                            <!-- <div class="mt-4 text-center text-base">_____</div>
                            ...
                            <div class="flex h-full items-stretch justify-center">
                                <div class="h-full w-[240px] border-2 border-black px-10 py-6 text-xl">
                                    <table class="h-full w-full border-collapse">
                                        <tbody>
                                            <tr>
                                                <td class="w-8 text-left align-middle">€</td>
                                                <td class="text-right align-middle">2 854,50</td>
                                            </tr>
                                            <tr>
                                                <td class="text-left align-middle">“</td>
                                                <td class="text-right align-middle">26,00</td>
                                            </tr>
                                            <tr>
                                                <td class="text-left align-middle">“</td>
                                                <td class="text-right align-middle">144,00</td>
                                            </tr>
                                            <tr>
                                                <td class="text-left align-middle">“</td>
                                                <td class="text-right align-middle">211,50</td>
                                            </tr>
                                            <tr>
                                                <td class="text-left align-middle">“</td>
                                                <td class="text-right align-middle">86,00</td>
                                            </tr>
                                            <tr>
                                                <td class="text-left align-middle">“</td>
                                                <td class="text-right align-middle">3 221,50</td>
                                            </tr>
                                            <tr>
                                                <td class="text-left align-middle">“</td>
                                                <td class="text-right align-middle">175,50</td>
                                            </tr>
                                            <tr>
                                                <td class="text-left align-middle">“</td>
                                                <td class="text-right align-middle">11,00</td>
                                            </tr>
                                            <tr class="border-t border-black">
                                                <td class="pt-2 text-left align-middle">€</td>
                                                <td class="pt-2 text-right align-middle">6 730,00</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                        ...
                        <table class="mt-3 w-full table-fixed border border-black text-base">
                            <thead>
                                <tr>
                                    ...
                                    <th v-for="denomination in cashDenominations" :key="denomination.key" class="border border-black p-1 text-center">
                                        {{ denomination.label }}
                                    </th>
        ...
        <div v-else-if="page === 4" class="flex h-full flex-col">
                ...
                <div class="flex h-full flex-col gap-4">
                    ...
                    <div class="flex flex-1 gap-4">
                        <div class="flex-1 border border-black/20 px-6 py-4">
                            <div class="space-y-4 text-center text-base leading-relaxed">
                                <p class="text-lg">
                                    Sie erhalten 10 Ordner und eine Menge Briefe. Diese sollen nach den Anfangsbuchstaben der Absender geordnet in den
                                    Ordnern abgelegt werden. Beim Sortieren der Briefe haben Sie festgestellt, dass die einzelnen Buchstaben des
                                    Alphabets prozentual unterschiedlich oft vorkommen (vgl. nebenstehende Zusammenstellung). Zum Schluss soll jeder
                                    Ordner die gleiche Menge Briefe enthalten.
                                </p>
                                ...
                                <table class="w-full border-collapse text-lg leading-tight">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td></td>
                                            <td></td>
                                            <td class="py-1 pl-8 text-left">
                                                <span class="tracking-[0.35em]">R P</span> und <span class="tracking-[0.35em]">Q</span>
                                            </td>
                                            <td class="py-1 pl-4 text-left">zusammen</td>
                                            <td class="py-1 text-center">=</td>
                                        ...
                                        <tr>
                                            ...
                                            <td></td>
                                            <td class="py-1 pl-8 text-left">
                                                <span class="tracking-[0.35em]">O</span> und <span class="tracking-[0.35em]">L</span>
                                            </td>
                                            <td class="py-1 pl-4 text-left">zusammen</td>
                                            <td class="py-1 text-center">=</td>
                                        ...
                                        <tr>
                                            ...
                                            <td></td>
                                            <td></td>
                                            <td class="border-b border-black py-1 pl-8 text-left">
                                                <span class="tracking-[0.35em]">M</span> und
            ...
            <div class="flex-[0.3] overflow-hidden px-6 pt-2 pb-4 font-serif">
            ...
            <div class="flex-[0.7] overflow-hidden px-6 pt-3 font-serif text-base">
                <div class="flex h-full flex-col gap-4">
                    ...
                    <div class="flex flex-1 gap-4">
                        <div class="flex-1 border border-black/20 px-6 py-4">
                            <div class="space-y-4 text-center text-xl leading-relaxed">
                                <p>Am Beginn des 1. Arbeitstages im Monat enthielt Ihre Portokasse Briefmarken für € 50,00.</p>
                                <p class="font-semibold">Die Anweisung lautet:</p>
                                <p>
                                    Es müssen abends auf der Post für € 20,00 neue Briefmarken gekauft werden, sobald der Markenbestand unter € 12,00
                                    gesunken ist.
                                </p>
                                <p>
                                    An welchen Arbeitstagen müssen Sie neue Briefmarken kaufen, wenn für die Postsendungen die in der Aufstellung
                                    genannten Briefmarken gebraucht worden sind?
                                </p>
                                <p>
                                    Bitte auf einem extra Blatt Nebenrechnungen vornehmen und dann die betreffenden Tage unten in der Tabelle
                                    anklicken (sodass ein „X“ erscheint).
                                </p>
                            ...
                            <tbody>
                                <tr>
                                    <th class="w-48 border border-black p-2 text-left" rowspan="2">
                                        ...
                                        <div class="mt-1 text-sm font-normal">Neue Briefmarken gekauft am (AT = Arbeitstag)</div>
                                    </th>
                                    <th v-for="day in 10" :key="`stamp-day-${day}`" class="border border-black p-1 text-center">{{ day }}.AT</th>
                                ...
                                <tr>
                                    <td v-for="day in 10" :key="`stamp-answer-${day}`" class="border border-black p-1 text-center">
                                        <button
                                            type="button"
                                            :class="[
                                                'h-10 w-full border border-black text-base font-semibold',
                                                answerTextClass('stamp_answer_days', day),
                                            ]"
        ...
        <div v-else-if="page === 6" class="flex h-full flex-col overflow-hidden">
            <div class="flex-[0.6] overflow-hidden px-6 pt-3 font-serif text-base">
                <div class="flex h-full flex-col gap-2">
                    ...
                    <div class="flex flex-1 items-stretch gap-4 overflow-hidden">
                        <div class="flex-[0.45] overflow-hidden border border-black/20 px-6 py-4">
                            <div class="flex h-full flex-col justify-between space-y-4 text-center text-xl leading-relaxed">
                                <p>
                                    Sie sollen Ihre Bekannten möglichst rasch benachrichtigen, dass die Feier zu Ihrem Geburtstag aus dienstlichen
                                    Gründen um einen Tag verlegt werden muss. Sie wissen, dass Sie alle jetzt zu Hause antreffen werden und sollen mit
                                    allen selbst sprechen, d.h. keinen anderen als Boten beauftragen. Einige haben Telefon, einige können Sie nur mit
                                    dem Fahrrad erreichen. Andere Beförderungsmittel sollen nicht benutzt werden. Die eigene Wohnung besitzt kein
                                    Telefon.
                                </p>
                                <p>
                                ...
                                <p>
                                    ...
                                    „Nachricht“ einzutragen.
                                </p>
                                <!-- <p class="font-semibold">In dieses Heft bitte keine Notizen machen!</p> -->
                                <!-- <div class="text-center text-lg">_____</div> -->
                            </div>
                        </div>
                        <div class="flex flex-[0.55] flex-col overflow-hidden border-2 border-black p-4">
                            <div class="mb-2 text-center text-xl font-bold underline underline-offset-4" style="letter-spacing: 0.4em">P l a n</div>
                            <div class="relative h-full min-h-0 w-full flex-1 select-none">
                                <svg class="absolute inset-0 h-full w-full" viewBox="0 0 800 400">
                                    <!-- Lines -->
                                    <line x1="120" y1="180" x2="400" y2="80" stroke="black" stroke-width="1" />
                                    <!-- M-Fr -->
                                    <line x1="120" y1="180" x2="400" y2="350" stroke="black" stroke-width="1" />
                                    <!-- M-Fu -->
                                    <line x1="400" y1="80" x2="680" y2="50" stroke="black" stroke-width="1" />
                                    <!-- Fr-B -->
                                    <line x1="400" y1="80" x2="680" y2="160" stroke="black" stroke-width="1" />
                                    <!-- Fr-H -->
                                    <line x1="400" y1="80" x2="680" y2="300" stroke="black" stroke-width="1" />
                                    <!-- Fr-S -->
                                    <line x1="400" y1="80" x2="400" y2="220" stroke="black" stroke-width="1" />
                                    <!-- Fr-Ho -->
                                    <line x1="680" y1="50" x2="680" y2="160" stroke="black" stroke-width="1" />
                                    <!-- B-H -->
                                    <line x1="680" y1="160" x2="680" y2="300" stroke="black" stroke-width="1" />
                                    <!-- H-S -->
                                    <line x1="680" y1="160" x2="400" y2="220" stroke="black" stroke-width="1" />
                                    <!-- H-Ho -->
                                    <line x1="680" y1="300" x2="400" y2="220" stroke="black" stroke-width="1" />
                                    <!-- S-Ho -->
                                    <line x1="680" y1="300" x2="400" y2="350" stroke="black" stroke-width="1" />
                                    <!-- S-Fu -->
                                    <line x1="400" y1="350" x2="400" y2="220" stroke="black" stroke-width="1" />
                                    <!-- Fu-Ho -->

                                    <!-- Numbers -->
                                    <text x="240" y="125" font-size="14" class="font-serif">5</text>
                                    <!-- M-Fr -->
                                    <text x="240" y="280" font-size="14" class="font-serif">20</text>
                                    <!-- M-Fu -->
                                    <text x="540" y="60" font-size="14" class="font-serif">5</text>
                                    <!-- Fr-B -->
                                    <text x="540" y="115" font-size="14" class="font-serif">5</text>
                                    <!-- Fr-H -->
                                    <text x="580" y="250" font-size="14" class="font-serif">12</text>
                                    <!-- Fr-S -->
                                    <text x="410" y="150" font-size="14" class="font-serif">5</text>
                                    <!-- Fr-Ho -->
                                    <text x="690" y="110" font-size="14" class="font-serif">5</text>
                                    <!-- B-H -->
                                    <text x="690" y="240" font-size="14" class="font-serif">5</text>
                                    <!-- H-S -->
                                    <text x="580" y="175" font-size="14" class="font-serif">8</text>
                                    <!-- H-Ho -->
                                    <text x="540" y="275" font-size="14" class="font-serif">5</text>
                                    <!-- S-Ho -->
                                    <text x="540" y="340" font-size="14" class="font-serif">15</text>
                                    <!-- S-Fu -->
                                    <text x="410" y="290" font-size="14" class="font-serif">15</text>
                                    <!-- Fu-Ho -->
                                </svg>

                                <!-- Draggable Names -->
                                <div
                                    class="absolute cursor-move border border-black bg-white px-4 py-1 font-serif text-xl whitespace-nowrap"
                                ...
                                <div
                                    class="absolute cursor-move border border-black bg-white px-4 py-1 font-serif text-xl"
                                ...
                                <!-- EIGENE WOHNUNG -->
                                <div
                                    class="absolute border-[3px] border-double border-black bg-white px-6 py-1 font-serif text-xl font-bold whitespace-nowrap"
            ...
            <div class="flex-[0.4] overflow-hidden px-6 pt-1 pb-4 font-serif">
                <div class="flex h-full flex-col">
                    <div class="border-t border-black" />
                    <div class="flex-1 overflow-hidden">
                        ...
                        <table class="mt-0.5 w-full table-fixed border-2 border-black text-sm">
                            ...
                            <tbody>
                                <tr v-for="row in routeRows" :key="`route-row-${row}`" class="h-9 border-b border-black">
                                    <td
                                    ...
                                    >
                                        <div class="flex items-center gap-2">
                                            ...
                                            <button
                                                v-if="
                                                    routeAssignments[`route-from-${row}`] &&
                                                    (row !== 1 || routeAssignments[`route-from-${row}`] !== 'Eigene Wohnung')
                                                "
                                                type="button"
                                                class="flex h-4 w-4 items-center justify-center rounded-full border border-black text-[10px] leading-none"
                                                @click="clearRouteAssignment(`route-from-${row}`)"
                                            >
                                                ×
                                            </button>
                                        </div>
                                    ...
                                    >
                                        <div class="flex items-center gap-2">
                                            ...
                                            <button
                                                ...
                                                type="button"
                                                class="flex h-4 w-4 items-center justify-center rounded-full border border-black text-[10px] leading-none"
                                                @click="clearRouteAssignment(`route-to-${row}`)"
                                            >
                                                ×
                                            </button>
                                        </div>
                                    ...
                                    <td class="border-r border-black px-2">
                                        <div class="flex items-center justify-end gap-3 pr-4">
                                            <input
                                                ...
                                                maxlength="3"
                                                :class="[
                                                    'h-6 w-16 border border-black text-center text-base',
                                                    answerTextClass('route_times', `route-time-${row}`),
                                                ]"
                                    ...
                                    <td class="px-2">
                                        <div class="flex items-center justify-end gap-2 pr-8">
                                            <input
                                                ...
                                                :class="[
                                                    'h-6 w-16 border border-black text-center text-base',
                                                    answerTextClass('route_times', `route-msg-${row}`),
                                                ]"
                                ...
                                <tr class="h-9 border-b border-black">
                                    <td class="border-r border-black px-2 pr-4 text-right" colspan="2">Gesamtzeit:</td>
                                    <td class="border-r border-black px-2">
                                        <div class="flex items-center justify-end gap-3 pr-4">
                                            <input
                                                ...
                                                :class="[
                                                    'h-6 w-16 border border-black text-center text-base',
                                                    answerTextClass('route_totals', 'totalWay'),
                                                ]"
                                    ...
                                    <td class="px-2">
                                        <div class="flex items-center justify-end gap-2 pr-8">
                                            <input
                                                ...
                                                :class="[
                                                    'h-6 w-16 border border-black text-center text-base',
                                                    answerTextClass('route_totals', 'totalMsg'),
                                                ]"
                                ...
                                <tr class="h-9">
                                    <td class="border-r border-black px-2 pr-4 text-right" colspan="2">anschließend zurück zur eigenen Wohnung:</td>
                                    <td class="border-r border-black px-2">
                                        <div class="flex items-center justify-end gap-3 pr-4">
                                            <input
                                                ...
                                                :class="[
                                                    'h-6 w-16 border border-black text-center text-base',
                                                    answerTextClass('route_totals', 'returnWay'),
                                                ]"
