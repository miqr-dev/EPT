...
const normTable = {
    ...
    L2: [30, 36, 41, 46, 50, 54, 58, 62, 65, 68, 71, 74, 78, 78, 78, 78, 78, 78, 78],
    'F-': [24, 30, 33, 35, 38, 41, 42, 45, 46, 49, 51, 54, 57, 58, 60, 63, 66, 71, 76],
    'F+': [33, 39, 43, 47, 50, 52.5, 55, 58, 62, 66, 75, 75, 75, 75, 75, 75, 75, 75, 75],
};
    ...
    <div class="p-4">
        ...
        <div class="mx-auto max-w-5xl space-y-8 rounded border border-border bg-background p-6 text-foreground shadow">
            ...
            <div v-if="!showTest && !isTestComplete" class="space-y-6 text-center">
                ...
                <p class="mx-auto max-w-3xl text-gray-600 dark:text-gray-400">
                    In diesem Verfahren geht es um ganz allgemeine Verhaltensweisen. Sie finden eine Anzahl von Fragen. Bei jeder Frage stehen mehrere
                    mögliche Antworten. Beantworten Sie bitte jede Frage nur mit einem Kreuz. Überlegen Sie nicht lange, sondern gehen Sie nach Ihrem
                    ersten Eindruck. Es gibt keine richtigen und falschen Antworten.
                </p>
                <Button @click="startTest" class="rounded px-8 py-3 text-lg font-semibold"> Test starten </Button>
            </div>

            <!-- Test Pages -->
            <div v-else-if="showTest && !isTestComplete">
                <h2 class="mb-6 text-xl font-bold text-gray-800 dark:text-gray-200">Seite {{ currentPage }} von {{ totalPages }}</h2>
                                ...
                                <td class="w-1/2 py-4 align-top">
                                    <div class="flex w-full flex-col gap-2">
                                        <label
                                            ...
                                            :key="idx"
                                            class="flex w-full cursor-pointer items-center justify-between rounded border px-3 py-2 transition hover:bg-gray-50 dark:hover:bg-gray-700"
                                            :class="{
                                                'border-blue-300 bg-blue-50 dark:border-blue-700 dark:bg-blue-900': userAnswers[q.number - 1] === idx,
                                            }"
                                        >
                                            ...
                                            <input
                                                ...
                                                :checked="userAnswers[q.number - 1] === idx"
                                                @change="selectAnswer(q.number - 1, idx)"
                                            />
                                        </label>
                                    </div>
                                </td>
            ...
            <div v-else-if="isTestComplete" class="space-y-8">
                ...
                <ul class="grid grid-cols-2 gap-x-6 text-sm text-gray-700 dark:text-gray-300">
                    <li>
                        L1 – Rohwert: <strong>{{ groupScores.L1 }}</strong>
                    </li>
                    <li>
                        L2 – Rohwert: <strong>{{ groupScores.L2 }}</strong>
                    </li>
                    <li>
                        F⁺ – Rohwert: <strong>{{ groupScores['F+'] }}</strong>
                    </li>
                    <li>
                        F⁻ – Rohwert: <strong>{{ groupScores['F-'] }}</strong>
                    </li>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full rounded border text-sm shadow">
                        <thead class="bg-gray-100 font-semibold text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                            <tr>
                    ...
                    <div class="mt-6 space-y-4">
                        <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">LMT – Leistungs-Motivations-Fragebogen</h3>
                        <table class="min-w-full rounded border text-sm shadow">
                ...
                <div class="text-center">
                    <Button @click="startTest" variant="outline" class="mt-6"> Test erneut starten </Button>
                </div>
            </div>
