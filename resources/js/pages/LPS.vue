...
type LpsPage11ResponseRow = { prompts: boolean[][] };
...
const COLUMN_LABELS = [1, 2, 3, 4, 5, 7, 8, 9, 11];
const PAGE_SECTIONS = [
  ...
  { title: 'Spalte 11', columnIndices: [8] },
];
...
watch(
  [page1Responses, page5Responses, page6Responses, page7Responses, page11Responses, pageIndex, totalElapsed, columnStates],
  () => {
    emit('update:answers', {
      ...
      page11_score: page11Score.value,
      page11_max_score: page11MaxScore.value,
    });
  },
  { deep: true },
);
...
type ColumnKey = keyof LpsPage1ResponseRow | 'col7' | 'col8' | 'col9' | 'col11';
...
function togglePage11Selection(rowIdx: number, promptIdx: number, optionIdx: number) {
...
const page11Score = computed(() =>
  page11Responses.value.reduce((total, response, idx) => {
    const solutions = lpsPage11Solutions[idx];
    if (!solutions) return total;
    return total + scorePage11Row(idx, solutions, response);
  }, 0),
);

const page11MaxScore = computed(() =>
  lpsPage11Solutions.reduce((total, solution) => {
    const rowTotal = solution.correctOptionIndices.reduce(
      (rowSum, idx) => (idx !== null && idx !== undefined ? rowSum + 1 : rowSum),
      0,
    );
    return total + rowTotal;
  }, 0),
...
const totalScore = computed(
  () => page1Score.value + page5Score.value + page6Score.value + page7Score.value + page11Score.value,
);
     ...
     <div class="mx-auto w-[1120px] max-w-none">
      ...
      <div v-if="!showTest" class="rounded-2xl border bg-background p-8 shadow-sm">
          <div class="mx-auto max-w-3xl space-y-4 text-center">
            <p class="text-base text-muted-foreground">
              Der {{ testLabel }}-Test umfasst acht Schritte. Beginnen Sie mit den Spalten 1 und 2, anschließend folgen
              die Spalten 3, 4, 5, 7, 8, 9 und 11 jeweils auf einer eigenen Seite. Arbeiten Sie jede Zeile von oben nach
              unten durch.
            </p>
      ...
      <div v-else class="space-y-4">
        ...
        <div v-else-if="pageIndex === 7" class="space-y-3">
          <div class="rounded-2xl border bg-background p-4 shadow-sm">
            <div class="mb-4 text-center text-[13px] font-extrabold tracking-wide text-foreground">Spalte 11</div>

            <div class="space-y-4">
              <div
                v-for="(row, rowIdx) in lpsPage11Rows"
                :key="`${row.id}-c11`"
                class="space-y-3"
              >
                <div class="grid gap-3 md:grid-cols-3">
                  ...
                  >
                    ...
                    <div v-if="prompt.options.length" class="flex flex-wrap justify-center gap-2">
                      <button
                        v-for="(option, optionIdx) in prompt.options"
                        :key="`${prompt.id}-${option}`"
                        type="button"
                        class="lps-number-chip"
                        :class="page11Responses[rowIdx]?.prompts?.[promptIdx]?.[optionIdx]
                          ? 'lps-number-chip--selected'
                          : ''"
                        :disabled="!isColumnInteractive('col11')"
                        :aria-pressed="page11Responses[rowIdx]?.prompts?.[promptIdx]?.[optionIdx]"
                        @click="togglePage11Selection(rowIdx, promptIdx, optionIdx)"
                      >
                        {{ option }}
                      </button>
                    </div>
                    <div v-else class="text-center text-xs text-muted-foreground">Noch keine Antwortoptionen hinterlegt.</div>
                  </div>
                </div>
              </div>
            </div>
