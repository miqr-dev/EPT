...
// Example norm table (expand as needed)
const normtable_18_30 = [
  { total: 0, SN: 1, PR: 70 },
  { total: 1, SN: 1, PR: 72 },
  // ... fill the rest from your image
  { total: 60, SN: 9, PR: 130 },
];
...
const groupStanines = computed(() =>
  groupScores.value.map((rw, groupIdx) => {
    ...
    return SN_WERTE_MATRIX[groupIdx][safeRW];
  })
);
                  ...
                  <tr v-for="(q, idx) in mrtQuestions" :key="idx" :class="userAnswers[idx] && q.correct.map(a => a.toUpperCase()).includes(userAnswers[idx]?.toUpperCase())
                    ...
                    <td class="px-2 py-1">
                      <span class="font-mono">
                        {{ userAnswers[idx]
                          ? q.options[userAnswers[idx].charCodeAt(0) - 65]
                          : '–'
                        }}
                      </span>
                    </td>
                    <td class="px-2 py-1">
                      <span class="font-mono">
                        {{q.correct.map(letter => q.options[letter.charCodeAt(0) - 65]).join(', ')}}
                      </span>
                    </td>
