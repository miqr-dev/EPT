...
function getAnswerNumber(ans: any, index: number) {
    return ans?.number != null ? ans.number : FPI_QUESTIONS[index]?.number;
}

function getAnswerValue(ans: any) {
    const value = ans?.user_answer ?? ans?.answer;
...
const normalizedAnswers = computed(() => {
    ...
    return props.answers.map((ans: any, index: number) => {
        const number = getAnswerNumber(ans, index);
        const question = FPI_QUESTIONS.find((q) => q.number === number);
        return {
            number,
            question: question?.text ?? ans.question,
            user_answer: getAnswerValue(ans),
            time_seconds: ans.time_seconds,
        };
    });
});

const fallbackMissingAnswerCount = computed(() => {
    const answerByNumber = new Map<number, string | null>();

    props.answers.forEach((ans: any, index: number) => {
        const number = getAnswerNumber(ans, index);
        if (typeof number === 'number' && Number.isFinite(number)) {
            answerByNumber.set(number, getAnswerValue(ans));
        }
    });

    return FPI_QUESTIONS.filter((question) => !answerByNumber.has(question.number) || !answerByNumber.get(question.number)).length;
});

const displayedMissingAnswerCount = computed(() => props.missingAnswerCount ?? fallbackMissingAnswerCount.value);
...
const chartCategories = [
    ...
    },
    {
        ...
        commentL: 'leistungsorientiert, aktiv schnell-handelnd<br>ehrgeizig-konkurrierend',
    ...
    {
        label: 'Aggressivität',
        commentL: 'aggressives Verhalten-spontan<br>und reaktiv,<br>sich durchsetzend',
    ...
    },
    {
        ...
        commentL: 'furcht vor Erkrankung<br>gesundheitsbewusst, sich schonend',
        commentR: 'wenig Gesundheitssorgen<br>gesundheitlich unbekümmert,<br>robust',
    },
    {
        label: 'Offenheit',
        commentL: 'Offenes Zugeben kleiner Schwächen und<br>alltäglicher Normverletzungen, ungeniert,<br>unkonventionell',
        commentR: 'an Umgangsnormen orientiert,<br>auf guten Eindruck bedacht,<br>mangelnde Selbstkritik,verschlossen<br>(Achtung bei Stanine 1-3)',
    },
    {
        label: 'Extraversion',
    ...
    {
        label: 'Emotionalität',
        commentL: 'emotional, labil, empfindlich ängstlich, viele Probleme und körperliche Beschwerden',
        ...
        <div class="main-grid">
            ...
            >
                ...
                <svg :width="gridWidth" :height="gridHeight" class="polyline-svg">
                    ...
                    <circle
                        v-for="(pt, idx) in drawableOutlierPoints"
                        :key="'outlier-' + idx"
                        :cx="pt.x"
                        :cy="pt.y"
                        r="8"
            ...
            <div class="footer-cell rohwert-cell">
                <div class="rohwert-box">{{ displayedMissingAnswerCount }}</div>
            </div>
